...
`config/logs.php`, список источников без пересечений:
...
- Уровень хранится в индексе как `uint8` — номер case в enum своего типа:
  `LaravelLogLevelEnum`, `NginxErrorLevelEnum`, `HttpStatusClassEnum`. Уровни ERROR и выше
  для watcher-а есть только у `laravel`.

## Индекс

Каталог `storage/framework/logs-index/<id>/`, где `id` = `sha1` пути файла
относительно `base_path()`.
...
2. Форматы, индекс, индексатор, тесты.
3. Чтение одного файла, поиск.

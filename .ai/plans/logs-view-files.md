...
- Индексы хранятся файлами в `storage/framework/logs-index`, пишутся и читаются через
  sconcur Files.
- Просмотр и скачивание. Удалять из UI можно файлы источников с `deletable: true`, кроме
  самого нового файла источника (см. «Удаление файлов»).
...
- Место под счётчики, спиннеры и «продолжить поиск» резервируется заранее. Размеры
  шрифтов не задаются.
- Стор переписан под новые типы `api-schema`. Выбранные файлы и фильтры живут в сторе, в
  URL их нет: поддержку query-строки убрали после шага 11. После изменений —
...
- Не по типу, а по источнику: ключ `deletable` в `config/module-logs.php` (по умолчанию
  false), включён у Laravel и Slogger.
- Самый новый файл источника (по `mtime`) удалить нельзя: воркеры держат его открытым, и
  записи до конца дня ушли бы в удалённый файл. `LogFileFinder` снимает с него `deletable`.
- `LogFileObject::deletable` → `can_delete` в списке файлов. `DeleteLogFileAction` под
  `LogIndexMutex` (ждёт 5 с) удаляет файл и индекс. Ответы: 404, 422, 409.
- Фронт: кнопка с `el-popconfirm` у каждого файла, у неудаляемых — `visibility: hidden`.
  Панель файлов — 320px, чтобы строка с размером и датой не переносилась.
...
12. Удаление файлов `laravel`. Сделано.
13. README, `make check`. Сделано.

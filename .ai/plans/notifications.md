...
| `watcher_ids` | json | пусто — все смотрители |
...
**Фильтр на канале, а не на смотрителе.** Смотритель — про наблюдение, а не про то, кому
рассказывать. Каналов мало, смотрителей много, и добавление канала не должно означать
...
└── Infrastructure/
    ...
    ├── Http/Requests/TelegramChannelRequest.php
...
1. **Каналы.** Таблица, модель, реестр типов, `TelegramSender`, маршрут теста.
   Проверяется кнопкой «тест» без единого смотрителя.
...
5. Фильтр по смотрителям — на канале.
6. `settings` шифруются, наружу токен уходит маской.
...
1. **Фильтр по смотрителям сразу или потом.** Можно начать с «канал получает всё» и
   добавить фильтр позже — но это правка контракта, а не добавление поля.
2. **Шифрование `settings`.** Предлагается да; цена — токен нельзя показать обратно.
3. **Повторные события по умолчанию выключены.** Если чат для дежурного, а не для
   истории, разумнее наоборот.
4. **Срок хранения исходящих — 30 дней**, как у инцидентов. Доставки нужны для разбора
   «почему молчало», и дольше месяца это едва ли кому-то интересно.

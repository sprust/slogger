...
различить: у `tracesSpike` порог и измеренный рост оба зовутся `growth_percent`, и по
именам ключей корзины определялись бы наоборот. Ключи документа знает только маппер типа —

...
из перечисленных, `statuses` — если его статус среди перечисленных. `v` — версия формата:
приёмник понимает только `2`, а встретив другую, пропускает такого смотрителя и пишет об
этом в лог, вместо того чтобы молча считать не то. PHP пишет текущую версию при любом
сохранении смотрителя, поэтому после выкладки смотрители версии `1` нужно пересохранить.
Приёмник выкладывается раньше панели: старый пропустит смотрителей версии `2`.
...
### Как сверяется статус

Статус трейса меняется по дороге: create приходит со `started`, финальный статус приезжает
апдейтом. Фильтр по статусу сверяется на каждой записи со статусом, который у трейса в этот
момент. Трейс считается один раз — при первой записи, поэтому в счёт попадает статус, с
которым трейс начался; длительность приходит с финальной записью и сверяется с финальным
статусом. Смотритель «спайк ошибок» по счёту трейсов так ошибок не увидит — это осознанное
решение, а не недосмотр.

В подпись свёртки статус не входит.
...
- **Счёт трейсов по финальному статусу** («спайк ошибок»): требует счёта по апдейтам, а не
  по созданиям — см. «Как сверяется статус».

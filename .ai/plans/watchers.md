...
| `payload` | object | `settings` — на что смотритель был настроен, `measured` — что увидел, `groups` — свёртка |
...
1. **Приёмник ждёт оба сервера** при остановке (`waitForShutdown` в `main.go`). Иначе
   ...
   пачки.
2. **Потолок линии — 720 элементов** (`RECEIVER_ELEMENT_CAP`), и это элементы, а не моменты:
   запись приёмника аддитивна и не читает документ, поэтому переоткрытое ведро приезжает
   вторым элементом с тем же `t`. У смотрителя с долгими трейсами один момент занимает

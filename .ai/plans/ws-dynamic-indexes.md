...
Нагрузка — ключи `TraceDynamicIndexResource`, то есть ровно то, что сегодня приезжает в
теле `412` (`Handler.php`). Фронт кладёт это в `pendingRequestStore.setData(...)` и
уже умеет такое показывать.
...
- Убрать `updateStats()` с рекурсивным `setTimeout`. Флаг `started` больше не нужен.
- Подписка на `sl-trace-indexes` поднимается один раз на приложение — там же, где
  создаётся клиент Echo, а не в компоненте: компонент размонтируется при уходе со
  страницы, а знать про идущее построение ЛК хочет всегда.

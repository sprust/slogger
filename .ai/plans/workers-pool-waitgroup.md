...
программы из supervisor — они стали группой `rabbitmq` мастера. После этой работы в
`supervisor.conf` останется три программы: `sl-sconcur-tasks`, `sl-sconcur-http-master`,
`sl-queue-slogger-dispatcher`.

**Мастер рассмотрен и отклонён.** Поднимать пул группой мастера смысла нет: панель
собирает снапшоты, которые раз в секунду пушит Go-сторона серверного рантайма
(`vendor/sconcur/sconcur/docs/admin-stats.ru.md`), а обычный artisan-воркер не пушит
ничего — в `status` группа была бы видна, в телеметрии нет. Остаётся ротация логов,
ради которой не стоит связывать жизненный цикл крона с жизненным циклом HTTP-пула.
Пул остаётся под supervisor.
...
```mermaid
flowchart TB
    sup["supervisor — программа sl-sconcur-tasks"]
...
считается в миллисекундах. Здесь всё иначе: три корутины, две из которых почти всё время
спят, и ни одного клиента, который ждёт ответа. Единственный, кому нужен поток вовремя,
— контролёр, а его бюджет измеряется не миллисекундами: при `shutdown_timeout_seconds`
= 20 и `stopwaitsecs` = 30 задержка в секунду не значит ничего.

Что даёт грубый квант: таймер дёргает прерывание VM раз в секунду, а не двести раз, и
парковок корутин на порядки меньше. Идеальной экономии тут, правда, и не было —
простаивающий пул не платит за таймер вовсе: пока PHP-поток запаркован в блокирующем
ожидании, таймер на паузе (`vendor/sconcur/sconcur/docs/coroutine-switching.ru.md`).
Так что 1000 мс — это про «не усложнять там, где не нужно», а не про экономию.

Что теряем: под вычислительным участком чужого тика контролёр просыпается с задержкой до
кванта. Худший случай реакции на `SIGTERM` — примерно квант плюс квант сна, то есть
~1.25 с вместо ~250 мс. На фоне 20-секундного дедлайна остановки это несущественно.

**Зачем она здесь.** Тик, ушедший в чистые вычисления, не уступает поток никому — а
уступать надо в первую очередь **контролёру**. Именно его тик в 250 мс доставляет
`SIGTERM` (обработчик `pcntl` выполняется только когда исполняется PHP-код) и опрашивает
канал управления. Без преемпции долгий невытесняемый участок внутри `tick()` откладывает
и то и другое, то есть бьёт ровно по graceful shutdown, который § 5.5 и § 5.9 строят.
Документация библиотеки называет этот случай прямо: «огромный `preg_match`,
`json_decode`» — на нём не помогает ничего, кроме преемпции. С ней задержка соседей
ограничена квантом преемпции, без неё — длиной всего вычислительного участка.

**Чего она не даёт.** Вытесняется только userland-код PHP. Нативный блокирующий вызов
(синхронный PDO, `curl`, родной `sleep`) прервать нечем — это отдельно оговорено в
`vendor/sconcur/sconcur/docs/worker-master.ru.md`, раздел «Зависший воркер». Поэтому
жёсткий дедлайн остановки (§ 5.9) остаётся обязательным, а не становится страховкой на
всякий случай.

**Как включаем.** Обязательно парой с выключением: таймер продолжает срабатывать до
`disablePreemption()` или конца процесса — это требование докблока
`Scheduler::enablePreemption()`. И под проверкой расширения: `Extension::get()`
в конструкторе зовёт `checkExtension()` и без `sconcur.so` бросает исключение, а пул
должен подниматься и в тестах.

```php
$preemption = $quantumMs > 0 && extension_loaded('sconcur');

if ($preemption) {
    Scheduler::get()->enablePreemption($quantumMs);
}

try {
    $waitGroup->iterate();          // ... дренаж пула
} finally {
    if ($preemption) {
...
**Цена, названная честно.** По
`packages/sconcur/sconcur-laravel/docs/fiber-safe-laravel-bridge.ru.md` § 6.4
физической изоляции PDO-соединения по корутинам в проекте нет. Без преемпции гонка
возможна только на связке «MySQL-транзакция + async-await внутри неё» (блокирующий PDO
корутину не усыпляет). С преемпцией поверхность шире: корутину могут припарковать на
любой границе опкодов, в том числе между двумя запросами одной транзакции, и вторая
корутина сходит в то же общее соединение.

Для сегодняшнего пула это умозрительно, и мой прежний аргумент против был сильнее, чем
заслуживает: **ни одна из двух задач к MySQL не обращается**. Крон диспатчит джобы в
Redis/RabbitMQ, монитор целиком на Mongo. Риск станет реальным ровно в тот момент, когда
в пуле появится задача с `DB::transaction()`.

Поэтому правило фиксируется в PHPDoc `TaskInterface` и в `docs/task-pool.ru.md`: **тик не
держит транзакцию MySQL на общем соединении**. Нужна транзакция — задача берёт
собственное соединение (вариант «б» из § 6.4 того же документа) либо пул поднимается с
`preemption_quantum_ms=0`.

## 6. Единственный конфиг — секция `tasks` в `config/sconcur.php`

Всё, что относится к пулу, живёт в одном месте рядом с `http_server` и `queue`:
...
## 7. `docker/supervisor/conf/supervisor.conf`

Секции `[program:sl-cron]` и `[program:sl-queue-trace-dynamic-indexes-monitor]`
удаляются, вместо них:

```ini
[program:sl-sconcur-tasks]
command=php /app/artisan sconcur:tasks:start
stdout_logfile=/var/log/supervisor/sl-sconcur-tasks-out.log
stderr_logfile=/var/log/supervisor/sl-sconcur-tasks-err.log
autostart=true
autorestart=true
startsecs=0
stopsignal=TERM
; больше SCONCUR_TASKS_SHUTDOWN_TIMEOUT_SECONDS — см. § 5.9
stopwaitsecs=30
```

Логи раньше были по файлу на программу, теперь один — поэтому каждая строка пула
префиксуется именем источника (`[cron]`, `[trace-dynamic-indexes]`, `[controller]`).
...
| `shutdown_timeout_seconds` больше `stopwaitsecs` | инвариант в конфиге, `supervisor.conf` и `DEPLOY.md`; дефолт supervisor — 10 с, строку надо задать явно (§ 5.9) |

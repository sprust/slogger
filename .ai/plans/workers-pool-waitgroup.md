...
flowchart TB
    ...
    mon -->|"awaitGroup — крон продолжает выполнять тики"| nested
```
...
`sconcur:tasks:start` рядом не выполнят `schedule:run` дважды. Имя лока — из
...
'tasks' => [
    ...
    // Автопереключение корутин: контролёр продолжает выполнять тики даже под
...
ту же минуту. Выключить группу можно только убрав её из списка — отдельного env-переключателя на

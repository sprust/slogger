...
`delays_ms` — лестница очередей ожидания. `confirm_publishes` включает подтверждение для
каждой публикации, `confirm_timeout_seconds` — сколько его ждать; отложенная публикация
подтверждается всегда, независимо от них.
...
Произвольная задержка округляется вверх до ближайшей объявленной, а всё, что выше
верхней ступени, прижимается к ней: адресовать необъявленную очередь значило бы
потерять джобу.

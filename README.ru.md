...
- Мастер и воркеры. `sconcur:servers:master:start` (поднимается supervisor'ом в контейнере `workers`) — супервизор пулов воркеров: спавнит `SCONCUR_HTTP_WORKER_COUNT` процессов как `php artisan sconcur:servers:http:start --masterPid=N`, перезапускает упавшие и зависшие, отдаёт телеметрию через панель. Пул — это элемент списка `groups` в конфиге мастера, и один мастер супервизит несколько непохожих пулов под одним локом и одним журналом; здесь пул один, с именем `http`. Все его воркеры слушают один порт через `SO_REUSEPORT`, поэтому nginx проксирует на них, ничего не зная о пуле. Одиночный воркер можно поднять и standalone — той же командой `sconcur:servers:http:start`. У `sconcur:servers:master:status` и `:reload` есть необязательный `--group=NAME` — действовать на один пул вместо всех.
- Корутинно-скоупленное приложение. При конкурентных фиберах модель Octane (клон приложения + своп глобального контейнера) небезопасна: соседние запросы видели бы состояние друг друга. Поэтому в `bootstrap/app.php` создаётся `SConcur\Laravel\Foundation\AsyncApplication` — drop-in наследник `Illuminate\Foundation\Application`, переносящий per-request состояние в контекст корутины: `request`, `auth`, `session`, `cookie`, оверлей конфига (`config()->set`), текущий маршрут, локаль, `View::share`, `defer`. Async-режим включается только внутри HTTP-воркера; для CLI, очередей и крона приложение ведёт себя как обычное Laravel-приложение.
- Неблокирующий I/O. Запросы к MongoDB и MySQL идут через драйверы SConcur (`Model::sconcur()`), поэтому фибер, ожидающий базу, отдаёт процесс другим запросам, а не блокирует его. Там, где одному запросу нужно сразу несколько обращений к шардам, они выполняются параллельно через `SConcur\WaitGroup` (поиск, графики, построение дерева).
- Про транзакции. Нельзя выполнять sconcur-async работу (Mongo, sconcur-SQL, HTTP-клиент, `Sleeper`) внутри открытой MySQL-транзакции: блокирующее PDO-соединение общее на процесс, и пока один фибер ждёт, другой может оказаться внутри его транзакции. Async-работу делай до `beginTransaction` или после `commit` (либо выноси в очередь).
- Очереди на том же рантайме. `QUEUE_CONNECTION` по умолчанию — `sconcur_rabbitmq`: драйвер очереди Laravel поверх AMQP-фичи SConcur плюс пул консьюмеров, который поднимается ещё одной группой того же мастера. Очереди `default`, `trace-tree` и `traces-clearing` раньше были тремя программами supervisor'а — пять, один и один процессов, каждый заблокирован на своей очереди; теперь это один процесс, читающий все три сразу с корутиной на доставку, а прежние счётчики процессов стали весами очередей в `SCONCUR_RABBITMQ_QUEUES`. Медленная джоба стоит одного сообщения, а не воркера. Формат на проводе — тот, что пишет `vladimir-yuldashev/laravel-queue-rabbitmq`: то же тело, те же свойства сообщения, тот же заголовок `laravel.attempts`, — поэтому джоба, отправленная любым из двух драйверов, читается и выполняется другим. Очередь трейсов `slogger` — четвёртая в этом же пуле. Её обслуживал `slogger:dispatcher:start` из внешнего пакета `slogger/laravel` — супервизор внутри супервизора, порождавший через Symfony Process свои `SLOGGER_DISPATCHER_QUEUE_WORKERS_COUNT` процессов `queue:work`; теперь его число воркеров стало весом очереди, а сама команда из supervisor убрана. Заодно эта работа появилась в панели телеметрии: мастер отдаёт сокет коллектора только своим воркерам, поэтому процессы, порождённые диспетчером, в неё не отчитывались. `SendTracesJob` несёт собственные `$tries` и `$backoff`, а значения самой джобы старше пуловых, поэтому её политика повторов не изменилась. Подробности — `packages/sconcur/sconcur-laravel/README.md`.

- Периодические задачи на том же рантайме. Крон и монитор динамических индексов были двумя программами supervisor'а — двумя процессами, в каждом свой вечный `while`, свой `sleep()` и свой самодельный способ остановиться (метка в кэше у крона, флаг в кэше у монитора). Теперь это две задачи одного пула корутин — группа `tasks` того же мастера, что держит HTTP-пул и консьюмеров очередей (`sconcur:tasks:start`, ровно один воркер): задача реализует только `tick()` — одну порцию работы, — а цикл, паузы, обработку исключений и остановку пул берёт на себя. Родной `sleep()` заморозил бы весь процесс, поэтому пауза идёт через `Sleeper` и усыпляет только свою корутину: пока монитор строит индекс в Mongo, крон продолжает тикать. Управление — `sconcur:tasks:stop [--task=]` и `sconcur:tasks:restart [--task=]`, они кладут команду в кэш, поэтому пулом можно управлять из другого контейнера, не зная его pid; прежние `cron:stop` и `trace-dynamic-indexes:monitor:stop` убраны. `cron:start` и `trace-dynamic-indexes:monitor:start` остались как способ поднять одну задачу отдельно. Телеметрию пул шлёт сам: Go-стороны, которая делает это у остальных воркеров, у него нет, поэтому `TaskPoolTelemetry` сэмплит RSS и CPU из `/proc` и отправляет снапшоты по тому же открытому контракту — unix-сокет, префикс длины, JSON. Тики пула при этом идут в секции `consumers` снапшота — тик для задачи то же, что доставка для консьюмера, — поэтому колонки дашборда In-flight / Handled / Refused наполняются сами; платой служит то, что сводные числа мастера считают тики вместе с настоящими доставками (выключается `report_ticks`). Это временное решение до появления отправки на Go-стороне. Группы, про которые панель молчит (например при остановленном пуле), дашборд дорисовывает нулями, а не теряет. Подробности — `packages/sconcur/sconcur-laravel/docs/task-pool.ru.md`.

...
Группы идут в том порядке, в каком их объявляет `config/sconcur.php`, а не в том, в каком ответила панель; воркеры — в том же порядке групп, затем по pid. Панель строит ответ из map, поэтому раньше строки прыгали под курсором. График можно навести на мастера, на одну группу или на один воркер — семплируется каждый источник на каждом опросе, так что переключение сохраняет уже собранную историю, — и предлагает In process, Finished/sec, среднюю длительность, CPU, RSS и горутины; по умолчанию включены Finished/sec и CPU. Finished/sec считается на клиенте по дельте счётчика между опросами. Если адрес панели или токен не настроены либо мастер не поднят, вкладка показывает рантайм как недоступный, а не падает с ошибкой.

...
Инцидент бесполезен тому, кто не смотрит в ЛК, поэтому его выносит наружу канал. Пока один тип, Telegram: бот пишет в чат, группу или канал. Канал сам говорит, о каком из трёх моментов он сообщает — инцидент заведён, под уже открытым появилось ещё одно событие, инцидент закрыт, — так что в один чат можно слать только заведения, а в другой всё подряд.

Каждый смотритель называет один канал, через который говорит, — он выбирается в его же форме, — и не назвать никакого тоже ответ: такой смотритель заводит инциденты в ЛК и никому о них не сообщает. Так двум командам можно слать каждой свою часть системы, вместо того чтобы все слышали всё. Выключенный канал, как и удалённый уже после того, как на него указали, заставляет смотрителя замолчать, а не копить отправки, которым некуда идти.

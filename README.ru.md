...
- Смотрители — настраиваемые правила, которые заводят инцидент, когда с системой что-то не так: растёт буфер, перестали приходить трейсы, их стало резко больше, они стали долгими.
...
Настраиваемые правила, которые следят за системой и заводят инцидент, когда одно из них нарушено. Пять типов: растёт буфер (`bufferOverflow`), приходят некорректные трейсы (`invalidBufferGrown`), трейсы перестали появляться (`noNewTraces`), их стало резко больше (`tracesSpike`), они стали долгими (`slowTraces`). Последние три принимают фильтр — сервисы, типы, теги, — так что смотритель может быть про одну часть системы, а не про всю сразу.

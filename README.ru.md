...
# WebSocket-пул SConcur: панель подписывается на него вместо опроса сборки дерева
# трейсов и динамических индексов. По умолчанию выключен, см. «Включение WebSocket-пула»
SCONCUR_WS_WORKER_COUNT=0   # 0 = пул выключен, панель опрашивает по таймеру
...
### Включение WebSocket-пула

Необязательно. С выключенным пулом панель спрашивает по таймеру, как и раньше; с
включённым сборка дерева трейсов и динамического индекса приезжает к ней сама.

Учётных данных в примерах нет — секрет, опубликованный в репозитории, ничего не
подписывает, а пул без них не стартует. Подойдёт любая пара случайных строк; ключ едет
в пути URL, так что держите его из букв и цифр.
...
SCONCUR_WS_APP_KEY=<ключ>      # публичный: его несёт браузер
SCONCUR_WS_APP_SECRET=<секрет> # подписывает подписки на каналы, за бэкенд не уходит
```

`frontend/.env`:

```dotenv
SCONCUR_WS_KEY=<тот же ключ>
```

Дальше пересобрать панель и перезапустить мастер, именно в таком порядке — ключ
запекается в бандл, а пул читает учётные данные при старте:
...
Если пул недоступен, ничего не ломается: каждое место, которое на него подписывается,
возвращается к опросу.

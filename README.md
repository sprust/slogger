...
# or a dynamic index being built. Off by default; see "Enabling the WebSocket pool" below
SCONCUR_WS_WORKER_COUNT=0   # 0 = pool off, the panel polls instead
...
### Enabling the WebSocket pool

Optional. With the pool off the panel asks again on a timer, the way it always did; with
it on, a trace tree and a dynamic index being built are pushed to it instead.

The credentials are not shipped in the example files — a secret published in a repository
signs nothing — and the pool refuses to start without them. Any two random strings will
do; the key travels in a URL path, so keep it to letters and digits.
...
SCONCUR_WS_APP_KEY=<the key>       # public: the browser carries it
SCONCUR_WS_APP_SECRET=<the secret> # signs channel subscriptions; never leaves the backend
```

`frontend/.env`:

```dotenv
SCONCUR_WS_KEY=<the same key>
```

Then rebuild the panel and restart the master, in that order — the key is baked into the
bundle, and the pool reads its credentials at startup:
...
Nothing breaks if the pool is unreachable: every view that subscribes falls back to
polling.
